  ...
  },
  {
    ...
    tiers: {
      good: {
        ...
        label: 'Key Coverage',
        tag: 'Good',
        features: ['Key area(s) provided with invisible speakers'],
      ...
      standard: {
        ...
        label: 'Main Living',
        tag: 'Standard',
        features: ['Main living area(s) including kitchen or other key area(s) provided with invisible speakers'],
        brands: 'Amina Loudspeakers, Stealth Acoustics'
      },
      better: {
        price: 28500,
        ...
        label: 'Whole Home',
        tag: 'Better',
        features: ['Replace all interior speakers with invisible speakers'],
        brands: 'Amina Loudspeakers, Stealth Acoustics'
      },
      best: {
        price: 38500,
  ...
  },
  {
    ...
    tiers: {
      good: {
        ...
        tag: 'Good',
        features: ['On-wall iPad mini in main living area'],
        brands: 'iPort, Apple'
      },
      standard: {
        price: 4800,
        ...
        tag: 'Standard',
        features: ['On-wall iPads in main living area and master suite'],
        brands: 'iPort, Apple'
      },
      better: {
        price: 7200,
...
export const RESIDENTIAL_EXTRAS = [
...
// ---- CONDO DATA ----
export let CONDO_CATEGORIES = [];
export let CONDO_EXTRAS = [];

export function initCondoData() {
  // Clone residential categories
  const newCategories = JSON.parse(JSON.stringify(RESIDENTIAL_CATEGORIES));
  const newExtras = JSON.parse(JSON.stringify(RESIDENTIAL_EXTRAS));
  
  // Remove categories not applicable to condos
  const removeIds = ['surveillance', 'outdoor', 'security', 'intercom'];
  const filteredCategories = newCategories.filter(c => !removeIds.includes(c.id));
  
  // Override networking for condos
  const netIdx = filteredCategories.findIndex(c => c.id === 'networking');
  if (netIdx !== -1) {
    filteredCategories[netIdx].tiers.good.features = [
      'WiFi 7 access points for full interior coverage and balcony',
      'Cloud managed pro router',
      'Managed PoE switch',
      'App-managed, remote access'
    ];
    filteredCategories[netIdx].tiers.good.brands = 'Ubiquiti UniFi, Ruckus, Access Networks';
  }
  
  // Update CONDO_CATEGORIES in place to preserve CONFIGS reference
  CONDO_CATEGORIES.length = 0;
  filteredCategories.forEach(c => CONDO_CATEGORIES.push(c));
  
  // Clear extras for condos
  CONDO_EXTRAS.length = 0;
  
  // DEBUG
  console.log('DEBUG initCondoData: RESIDENTIAL_CATEGORIES length:', RESIDENTIAL_CATEGORIES.length);
  console.log('DEBUG initCondoData: CONDO_CATEGORIES length:', CONDO_CATEGORIES.length);
}
...
// DEBUG
console.log('DEBUG module load: RESIDENTIAL_CATEGORIES length:', RESIDENTIAL_CATEGORIES.length);
console.log('DEBUG module load: CONDO_CATEGORIES length:', CONDO_CATEGORIES.length);
console.log('DEBUG module load: CONFIGS.residential.categories length:', CONFIGS.residential.categories.length);
console.log('DEBUG module load: CONFIGS.condo.categories length:', CONFIGS.condo.categories.length);

export const SECTION_ORDER = ['Infrastructure', 'Audio', 'Video', 'Control & Automation', 'Lighting & Shades', 'Security', 'Custom'];
